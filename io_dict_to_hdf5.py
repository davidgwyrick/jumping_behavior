...
def load_MAP_seqs_h5(fpath):
    #Load into nested dictionary
    results = load(fpath)
    
    #Extract MAP sequence as list instead of dictionary
    nTrials = len(results['trMAPs'].keys())
    trMAPs = []; trPosteriors = []; trMasks = []
    for iTrial in range(nTrials):
        trMAPs.append(results['trMAPs'][str(iTrial)])
        trPosteriors.append(results['trPosteriors'][str(iTrial)])
        trMasks.append(results['trMasks'][str(iTrial)])
        
    #Get model parameters
    arhmm_params = results['arhmm_params']
    opt = results['hyperparams']
    
    return trMAPs, trPosteriors, trMasks, arhmm_params, opt
